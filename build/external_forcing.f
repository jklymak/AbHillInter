...
C     JMK: Mean flow in x direction.
C     Needs to be slow for low f....
      spinup = 50.*3600.*1.01
C      spinup = 100.
C      U0=0.1*MIN(myTime/spinup,1.)
      U0=0.1
C      WRITE(msgBuf,'(E10.4)')  U0
C      CALL PRINT_MESSAGE( msgBuf, standardMessageUnit, SQUEEZE_RIGHT,
C     $     myThid )
...
C     JMK: add a body force
C      WRITE(msgBuf,'(E10.4)')  fCori(10,1,bi,bj)
C      CALL PRINT_MESSAGE( msgBuf, standardMessageUnit, SQUEEZE_RIGHT,
C     $     myThid )
      DO j=1,sNy+1
         DO i=0,sNx+1
