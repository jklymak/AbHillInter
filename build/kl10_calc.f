...
C     &     KLnRi
      Real*8    KLdumpFreq, KLtaveFreq, KLviscMax
      COMMON /KL10_PARMS_R/
     &     KLdumpFreq, KLtaveFreq, KLviscMax

      Real*8 KLviscAr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 KLdiffKr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 KLeps (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C     iMin, iMax, jMin, jMax :: array computation indices
      ...
      Real*8     KLviscTmp, rhot, tempu
      Real*8     b0, buoyFreqf, buoyFreqc, KLviscold,zsum,zsums
      Real*8     rhoS(1:Nr),RS(1:Nr)
      Real*8     dzp,ec,ep,es,epss(-1:0),epsw(-1:0),dz,KTemp
c     Real*8     bF(1:Nr)
c     Real*8     theta_mcb(1:Nr),theta_mcb3(1:Nr)
C     === Local variables ===
...
C N at surface = zero or uses gradient
     ...
     &              (rhoS(1) - rhoS(2))*recip_drC(2),0.D0)
c           b0 = 0.
            DO di=-1,0
               epss(di)=0.0
               epsw(di)=0.0
...
C                  buoyFreqf = 0.
               ENDIF
               buoyFreqf = MAX(buoyFreqf,0.D0) ! not < 0
